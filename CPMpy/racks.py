...
class RacksExample:
    """ non-lifted version of rack example """
    ...
    def __init__(self,maxnrofobjects):
    ...
    def build_model(self):
        ...
        self.OBJECTCOUNT = cpmpy.intvar(0,self.N,shape=MAXTYPES, name="objectcount")
        self.model += self.OBJECTCOUNT[0] == 0

        # objecttype
        for i in range(1,self.MAXOBJ):
            cond = (i <= self.NROFOBJECTS)
            expr = (self.OBJECTTYPE[i]!=RackConfiguration.UNUSED)
        ...
        for i in range(1,MAXTYPES):
            count = cpmpy.intvar(0,self.N,shape=self.MAXOBJ)
            self.model += count[0] == 0
            for j in range(1,self.MAXOBJ):
                cond = self.OBJECTTYPE[j] == i
                self.model += (cond).implies(count[j] == 1)
                cond = self.OBJECTTYPE[j] != i
    ...
    def define_rack_frame_association(self):
        ...
        self.RACK_FRAME_COUNT = cpmpy.intvar(0,self.N,shape=self.MAXOBJ, name="framecount")
        ...
        for i in range(1,self.MAXOBJ):
            self.ASSIGNED_TO_RACK[i] = cpmpy.intvar(0,self.N,self.MAXOBJ)
            self.model += self.ASSIGNED_TO_RACK[i][0] == 0
            for j in range(1,self.MAXOBJ):
                cond = (self.RACK_OF[j]==i)
                self.model += (cond).implies(self.ASSIGNED_TO_RACK[i][j] == 1)
                cond = (self.RACK_OF[j]!=i)
        ...
        for i in range(1,self.MAXOBJ):
            self.model += (self.OBJECTTYPE[i]==RackConfiguration.RACKSINGLE).implies(4 == self.RACK_FRAME_COUNT[i])

        for i in range(1,self.MAXOBJ):
            self.model += (self.OBJECTTYPE[i]==RackConfiguration.RACKDOUBLE).implies(8 == self.RACK_FRAME_COUNT[i])
    ...
    def define_frame_module_association(self):
        ...
        self.FRAME_MODULE_COUNT = cpmpy.intvar(0,self.N,shape=self.MAXOBJ, name="framemodulecount")
        ...
        for i in range(1,self.MAXOBJ):
            self.ASSIGNED_TO_FRAME[i] = cpmpy.intvar(0,self.N,self.MAXOBJ)
            self.model += self.ASSIGNED_TO_FRAME[i][0] == 0
            for j in range(1,self.MAXOBJ):
                cond = (self.FRAME_OF[j]==i)
                self.model += (cond).implies(self.ASSIGNED_TO_FRAME[i][j] == 1)
                cond = (self.FRAME_OF[j]!=i)
        ...
        for i in range(1,self.MAXOBJ):
            self.model += (self.OBJECTTYPE[i]==RackConfiguration.FRAME).implies(5 >= self.FRAME_MODULE_COUNT[i])
    ...
    def define_element_module_association(self):
        ...
        self.ELEMENT_MODULE_COUNT = cpmpy.intvar(0,self.N,shape=self.MAXOBJ, name="elementmodulecount")
        ...
        cond = {}
        for i in range(1,self.MAXOBJ):
            self.ASSIGNED_TO_ELEMENT[i] = cpmpy.intvar(0,self.N,self.MAXOBJ)
            self.model += self.ASSIGNED_TO_ELEMENT[i][0] == 0
            for j in range(1,self.MAXOBJ):
                cond = (self.ELEMENT_OF[j]==i)
                self.model += (cond).implies(self.ASSIGNED_TO_ELEMENT[i][j] == 1)
                cond = (self.ELEMENT_OF[j]!=i)
    ...
    def element_modules_cardinality_constraints(self):
        ...
        for i in range(1,self.MAXOBJ):
            self.model += (self.OBJECTTYPE[i]==RackConfiguration.ELEMENTA).implies(1 == self.ELEMENT_MODULE_COUNT[i])

        for i in range(1,self.MAXOBJ):
            self.model += (self.OBJECTTYPE[i]==RackConfiguration.ELEMENTB).implies(2 == self.ELEMENT_MODULE_COUNT[i])

        for i in range(1,self.MAXOBJ):
            self.model += (self.OBJECTTYPE[i]==RackConfiguration.ELEMENTC).implies(3 == self.ELEMENT_MODULE_COUNT[i])

        for i in range(1,self.MAXOBJ):
            self.model += (self.OBJECTTYPE[i]==RackConfiguration.ELEMENTD).implies(4 == self.ELEMENT_MODULE_COUNT[i])

    def constraint_moduleII_requires_moduleV(self):

        # count number of moduleII for frame
        self.MODULEII_ASSIGNED_TO_FRAME = {}
        self.FRAME_MODULEII_COUNT = cpmpy.intvar(0,self.N,shape=self.MAXOBJ, name="framemoduleiicount")
        self.model += self.FRAME_MODULEII_COUNT[0] == 0
        for i in range(1,self.MAXOBJ):
            self.MODULEII_ASSIGNED_TO_FRAME[i] = cpmpy.intvar(0,self.N,self.MAXOBJ)
            self.model += self.MODULEII_ASSIGNED_TO_FRAME[i][0] == 0
            for j in range(1,self.MAXOBJ):
                cond = (self.FRAME_OF[j]==i)&(self.OBJECTTYPE[j]==RackConfiguration.MODULEII)
                self.model += (cond).implies(self.MODULEII_ASSIGNED_TO_FRAME[i][j] == 1)
                cond = (self.FRAME_OF[j]!=i)|(self.OBJECTTYPE[j]!=RackConfiguration.MODULEII)
        ...
        self.FRAME_MODULEV_COUNT = cpmpy.intvar(0,self.N,shape=self.MAXOBJ, name="framemodulevcount")
        self.model += self.FRAME_MODULEV_COUNT[0] == 0
        for i in range(1,self.MAXOBJ):
            self.MODULEV_ASSIGNED_TO_FRAME[i] = cpmpy.intvar(0,self.N,self.MAXOBJ)
            self.model += self.MODULEV_ASSIGNED_TO_FRAME[i][0] == 0
            for j in range(1,self.MAXOBJ):
                cond = (self.FRAME_OF[j]==i)&(self.OBJECTTYPE[j]==RackConfiguration.MODULEV)
                self.model += (cond).implies(self.MODULEV_ASSIGNED_TO_FRAME[i][j] == 1)
                cond = (self.FRAME_OF[j]!=i)|(self.OBJECTTYPE[j]!=RackConfiguration.MODULEV)
    ...
    def build_from_solution(self):
        ...
        for i in range(1,self.N+1):
            configuration.create_object(i,1,objecttypes[i])
    ...
    def solve_and_print_solution(self):
        if (self.solve()):
            ...
            cv = configuration.get_violated_constraints()
            print(cv)
    ...
    def solve_and_show_solution(self):
        ...
        if configuration is not None:
            ...
            if len(cvs)>0:
                print(f"violated constraints {cvs}")
    ...
    def testcase_two_rack_double():
        ...
        lr.model += lr.OBJECTTYPE[2] == RackConfiguration.RACKDOUBLE
        lr.solve_and_print_solution()

    @staticmethod
    def testcase_21elementA():
        lr = RacksExample(21 + 21 + 1 + 7)
        for i in range(1,21+1):
            lr.model += lr.OBJECTTYPE[i] == RackConfiguration.ELEMENTA
        lr.solve_and_print_solution()
